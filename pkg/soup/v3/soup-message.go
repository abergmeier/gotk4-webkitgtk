...
const (
	...
	// and proxy authentication. Note that Message::authenticate signal will
...
// after an unacceptable TLS certificate has been received. You can return TRUE
...
// soup_auth_authenticate() or soup_auth_cancel() are called.
...
// message. (After Message::got_body).
...
// ConnectGotBody is emitted after receiving the complete message request body.
//
...
// ConnectRequestCertificate is emitted during the msg's connection
// TLS handshake when tls_connection requests a certificate from
// the client. You can set the client certificate by calling
// soup_message_set_tls_client_certificate() and returning TRUE. It's
// possible to handle the request asynchornously by returning TRUE and call
// soup_message_set_tls_client_certificate() later once the certificate is
// available. Note that this signal is not emitted if Session::tls-interaction
// was set, or if soup_message_set_tls_client_certificate() was called before
// the connection TLS handshake started.
...
// TLS handshake when tls_connection requests a certificate password from
// the client. You can set the certificate password on password, then call
// soup_message_tls_client_certificate_password_request_complete() and return
// TRUE to handle the signal synchronously. It's possible to handle the request
// asynchornously by calling g_object_ref() on password, then returning TRUE and
// call soup_message_tls_client_certificate_password_request_complete() later
// after setting the password on password. Note that this signal is not emitted
// if Session::tls-interaction was set.
...
// given encoded_form to uri via method. If method is "GET", it will include the
// form data into uri's query field, and if method is "POST" or "PUT", it will
// be set as request body. This function takes the ownership of encoded_form,
// that will be released with g_free() when no longer in use. See also
// soup_form_encode(), soup_form_encode_hash() and soup_form_encode_datalist().
//
// The function takes the following parameters:
//
...
//
//   - method: HTTP method for the created request.
//   - uri: destination endpoint (as a #GUri).
//
// The function returns the following values:
//
//   - message: new Message.
...
// The function takes the following parameters:
//
//   - baseUri: destination endpoint (as a #GUri).
//
// The function returns the following values:
//
//   - message: new Message.
...
// ConnectionID returns the unique idenfier for the last connection used. This
// may be 0 if it was a cached resource or it has not gotten a connection yet.
//
// The function returns the following values:
//
...
// FirstParty gets msg's first-party #GUri.
//
// The function returns the following values:
//
...
// navigation. Used for same-site policy checks.
//
// The function returns the following values:
//
...
// Metrics: get the MessageMetrics of msg. If the flag
// SOUP_MESSAGE_COLLECT_METRICS is not enabled for msg this will return NULL.
//
// The function returns the following values:
//
//   - messageMetrics (optional) or NULL.
//
...
// RemoteAddress: get the remote Address of the connection associated with
// the message. The returned address can be NULL if the connection hasn't
// been established yet, or the resource was loaded from the disk cache.
// In case of proxy connections, the remote address returned is a Address. If
// Session::remote-connetable is set the returned address id for the connection
...
// SetIsOptionsPing: set whether msg is intended to be used to send OPTIONS *
// to a server. When set to TRUE, the path of Message:uri will be ignored and
...
// SetRequestBody: set the request body of a Message. If content_type is
// NULL and stream is not NULL the Content-Type header will not be changed if
// present. The request body needs to be set again in case msg is restarted (in
// case of redirection or authentication).
//
// The function takes the following parameters:
//
...
// SetRequestBodyFromBytes: set the request body of a Message from #GBytes.
...
// when a client certificate is requested during the TLS handshake. You can
// call this as a response to Message::request-certificate signal, or before the
// connection is started. If certificate is NULL the handshake will continue
// without providing a GTlsCertificate. Note that the Certificate set by this
// function will be ignored if Session::tls-interaction is not NULL.
//
// The function takes the following parameters:
//
...
// re-send it with the new URI, you need to send it again.
//
// The function takes the following parameters:
//
//   - uri: new #GUri.
...
// signal, to notify msg that the Password has already been updated.
