...
//
// Size metrics are expressed in bytes and aree updated while the Message is
...
// and TLS handshake. It will be 0 if no network connection was required to
// fetch the resource (a persistent connection was used or resource was loaded
...
// establish the connection to the server. It will be 0 if no network connection
// was required to fetch the resource (a persistent connection was used or
// resource was loaded from the local disk cache).
//
// The function returns the following values:
//
...
// lookup name for the resource. It will be 0 if no domain lookup was required
// to fetch the resource (a persistent connection was used or resource was
...
// RequestBodyBytesSent: get the number of bytes sent to the network for
// the request body. This is the size of the body sent, after encodings
// are applied, so it might be greater than the value returned by
// soup_message_metrics_get_request_body_size(). This value is available
// right before Message::wrote-body signal is emitted, but you might get an
...
// RequestBodySize: get the request body size in bytes. This is the size of
// the original body given to the request before any encoding is applied.
...
// request headers. This value is available right before Message::wrote-headers
...
// for the response body. This value is available right before Message::got-body
// signal is emitted, but you might get an intermediate value if called before.
...
// ResponseBodySize: get the response body size in bytes. This is
// the size of the body as given to the user after all encodings
// are applied, so it might be greater than the value returned by
// soup_message_metrics_get_response_body_bytes_received(). This value is
// available right before Message::got-body signal is emitted, but you might get
...
// bytes of the response from the server or the local disk cache. In case of
// load failure, this returns the time immediately before the fetch is aborted.
//
// The function returns the following values:
//
...
// network for the response headers. This value is available right before
// Message::got-headers signal is emitted, but you might get an intermediate
// value if called before. For resources loaded from the disk cache this value
// is always 0.
//
// The function returns the following values:
//
...
// handshake. It will be 0 if no TLS handshake was required to fetch the
// resource (connection was not secure, a persistent connection was used or
// resource was loaded from the local disk cache).
//
// The function returns the following values:
//
