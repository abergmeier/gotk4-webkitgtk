...
//
func (webView *WebView) CallAsyncJavascriptFunction(ctx context.Context, body string, length int, arguments *glib.Variant, worldName, sourceUri string, callback gio.AsyncReadyCallback) {
...
//   - callback (optional) to call when the script finished.
//
func (webView *WebView) EvaluateJavascript(ctx context.Context, script string, length int, worldName, sourceUri string, callback gio.AsyncReadyCallback) {
	var _arg0 *C.WebKitWebView      // out
	...
	_arg1 = (*C.char)(unsafe.Pointer(C.CString(script)))
	defer C.free(unsafe.Pointer(_arg1))
	_arg2 = C.gssize(length)
	if worldName != "" {
